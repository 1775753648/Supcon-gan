...
def parse_args():
    ...
    parser.add_argument('--aug2', default='simclr_normal', type=str, help='Augmentation (default: hfrt)')
    ...
    parser.add_argument('--evaluate_every', help='', default=5000, type=int)
...
def train(P, opt, train_fn, models, optimizers, train_loader, logger):
    ...
    metrics['fixed_gen'] = FixedSampleGeneration(generator, volatile=P.no_gif)
    if not P.no_fid:
    ...
    for step in range(P.starting_step, opt['max_steps']+1):
        ...
        for i in range(opt['n_critic']):
            ...
            d_loss, aux = train_fn["D"](P, discriminator, opt, images, gen_images,labels)
        ...
        if step % P.evaluate_every == 0 :
            ...
            if fid_score:
                fid_avg = fid_score.update(step, generator)
            ...
            #aug_grid = (aug_grid * 255).astype(np.uint8)
...
def worker(gpu, P):
    ...
    train_loader = cycle(train_loader, distributed=False)
    ...
    P.augment_fn2 = get_augment(mode=P.aug2).cuda()
