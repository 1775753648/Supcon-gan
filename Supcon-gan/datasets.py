...
def get_dataset(dataset):
    ...
    if dataset == 'cifar10' or dataset == 'cifar100':
        ...
        if dataset == 'cifar10':
            ...
            num_classes = 10  # 选择前 10 个类别
        else:
            data = datasets.CIFAR100
            num_classes = 10  # 选择前 10 个类别

        # 获取完整数据集
        full_train_set = data(DATA_PATH, train=True, transform=transform, download=True)

        # 选择前 num_classes 个类别的样本
        selected_train_set = []
        for i in range(len(full_train_set)):
            image, label = full_train_set[i]
            if label < num_classes:
                selected_train_set.append((image, label))

        # 创建数据集
        train_set = torch.utils.data.Subset(selected_train_set, range(len(selected_train_set)))
        ...
        print(f"Length of the training dataset: {len(test_set)}")
